...
CDGFile::CDGFile() {
        cout << "CDGFile() constructor called" << endl;
}
...
int CDGFile::readNext() {
    ...
    //TODO: fix what happens when stream is over
    file.read((char*)&subcode, sizeof(Subcode));
    if (file.eof()){
        byteCount = 20000000; //Hack to cause the while loop not to hog all cpu
        cout << "CDGFile Stream finished." << endl;
        return;
    }else if (file.fail()) {
        cout << "CDGFile ERROR: Cannot read the file..." << endl;
        return;
    }
...
void CDGFile::resetCDG() {
    byteCount = 0;
    
    // Also reset the file stream
    file.clear();
...
int CDGFile::open(const char *filename) {
    ...
    if (file.fail()) {
        cout << "CDGFile ERROR: Cannot open the file..." << endl;
        exit(1);
    }
}
